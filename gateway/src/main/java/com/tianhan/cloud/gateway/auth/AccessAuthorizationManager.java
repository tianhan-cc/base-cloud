...
public class AccessAuthorizationManager implements ReactiveAuthorizationManager<AuthorizationContext> {
    ...
    public Mono<AuthorizationDecision> check(Mono<Authentication> authentication, AuthorizationContext object) {
        return authentication.map(var -> check((UserDetail) var.getPrincipal(), object.getExchange())).defaultIfEmpty(new AuthorizationDecision(false));
    }

    public AuthorizationDecision check(UserDetail user, ServerWebExchange exchange) {
